 ...
 */
int bmpReader::readBitmapFileHeader(FILE *fp, BITMAPFILEHEADER *bfh)
{
    int rc;

    rc = endianConverter.readUINT16little(fp, &(bfh->type));
    if (rc != 0)
	return rc;

    rc = endianConverter.readUINT32little(fp, &(bfh->size));
    if (rc != 0)
	return rc;

    rc = endianConverter.readINT16little(fp, &(bfh->xHotspot));
    if (rc != 0)
	return rc;

    rc = endianConverter.readINT16little(fp, &(bfh->yHotspot));
    if (rc != 0)
	return rc;

    rc = endianConverter.readUINT32little(fp, &(bfh->offsetToBits));
    return rc;
}
 ...
 */
int bmpReader::readBitmapArrayHeader(FILE *fp, BITMAPARRAYHEADER *bah)
{
    int rc;

    rc = endianConverter.readUINT16little(fp, &(bah->type));
    if (rc != 0)
	return rc;
    rc = endianConverter.readUINT32little(fp, &(bah->size));
    if (rc != 0)
	return rc;
    rc = endianConverter.readUINT32little(fp, &(bah->next));
    if (rc != 0)
	return rc;
    rc = endianConverter.readUINT16little(fp, &(bah->screenWidth));
    if (rc != 0)
	return rc;
    rc = endianConverter.readUINT16little(fp, &(bah->screenHeight));
    return rc;
}
 ...
 */
int bmpReader::readBitmapHeader(FILE *fp, BITMAPHEADER *bh)
{
     ...
     */
    rc = endianConverter.readUINT32little(fp, &(bh->size));
    if (rc != 0)
	return rc;
    bytesRead = 4;
    ...
    if (oldFormat)
    {
	rc = endianConverter.readUINT16little(fp, &tempVal);
	if (rc != 0)
	    return rc;
	bh->width = tempVal;
	bytesRead += 2;
    ...
    {
	rc = endianConverter.readINT32little(fp, &(bh->width));
	if (rc != 0)
	    return rc;
	bytesRead += 4;
    }
    ...
    {
	rc = endianConverter.readINT32little(fp, &(bh->height));
	if (rc != 0)
	    return rc;
	bytesRead += 4;
    }
     ...
     */
    rc = endianConverter.readUINT16little(fp, &(bh->numBitPlanes));
    if (rc != 0)
	return rc;
    bytesRead += 2;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT16little(fp, &(bh->numBitsPerPlane));
    if (rc != 0)
	return rc;
    bytesRead += 2;
    if (bytesRead >= bh->size)
     ...
     */
    rc = endianConverter.readUINT32little(fp, &(bh->compressionScheme));
    if (rc != 0)
	return rc;
    bytesRead += 4;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT32little(fp, &(bh->sizeOfImageData));
    if (rc != 0)
	return rc;
    bytesRead += 4;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT32little(fp, &(bh->xResolution));
    if (rc != 0)
	return rc;
    bytesRead += 4;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT32little(fp, &(bh->yResolution));
    if (rc != 0)
	return rc;
    bytesRead += 4;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT32little(fp, &(bh->numColorsUsed));
    if (rc != 0)
	return rc;
    bytesRead += 4;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT32little(fp, &(bh->numImportantColors));
    if (rc != 0)
	return rc;
    bytesRead += 4;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT16little(fp, &(bh->resolutionUnits));
    if (rc != 0)
	return rc;
    bytesRead += 2;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT16little(fp, &(bh->padding));
    if (rc != 0)
	return rc;
    bytesRead += 2;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT16little(fp, &(bh->origin));
    if (rc != 0)
	return rc;
    bytesRead += 2;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT16little(fp, &(bh->halftoning));
    if (rc != 0)
	return rc;
    bytesRead += 2;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT32little(fp, &(bh->halftoningParam1));
    if (rc != 0)
	return rc;
    bytesRead += 4;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT32little(fp, &(bh->halftoningParam2));
    if (rc != 0)
	return rc;
    bytesRead += 4;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT32little(fp, &(bh->colorEncoding));
    if (rc != 0)
	return rc;
    bytesRead += 4;
    if (bytesRead >= bh->size)
    ...
    rc = endianConverter.readUINT32little(fp, &(bh->identifier));
    if (rc != 0)
	return rc;
    bytesRead += 4;
    if (bytesRead >= bh->size)
 ...
 */
int bmpReader::readRgb(FILE *fp, RGB *rgb, int numBytes)
{
    int rc;

    if (numBytes == 0)
	return 0;
    rc = endianConverter.readUINT8little(fp, &(rgb->blue));
    if (rc != 0)
	return rc;

    if (numBytes == 1)
	return 0;
    rc = endianConverter.readUINT8little(fp, &(rgb->green));
    if (rc != 0)
	return rc;

    if (numBytes == 2)
	return 0;
    rc = endianConverter.readUINT8little(fp, &(rgb->red));
    if (rc != 0)
	return rc;
 ...
 */
int bmpReader::readColorTable(FILE *fp, RGB *rgb, int numEntries, int numBytesPerEntry)
{
    int i, rc;
 ...
 */
int bmpReader::readBitsUncompressed(FILE *fp, RGB *image, int width, int height,
			 int depth, RGB *colorTable)
{
	...
	for (row = height; row > 0; row--)
	{
	    ...
	    {
		rc = endianConverter.readUINT8little(fp, &temp);
		if (rc != 0)
		    return rc;
 ...
 */
int bmpReader::readMaskBitsUncompressed(FILE *fp, char *image, int width, int height)
{
    UINT8 temp;
    int   rc, padBytes, i;
 ...
 */
void bmpReader::reflectYRGB(RGB *image, int width, int height)
{
    int row, col;
    RGB temp;
 ...
 */
void bmpReader::reflectYchar(char *image, int width, int height)
{
    int row, col;
    char temp;
 ...
 */
int bmpReader::readSingleImageBMP(FILE *fp, RGB **argb, UINT32 *width, UINT32 *height)
{
 ...
 */
int bmpReader::readSingleImageICOPTR(FILE *fp, char **xorMask, char **andMask,
 ...
 * masks.
 */
int bmpReader::readSingleImageColorICOPTR(FILE *fp, RGB **argb, char **xorMask,
 ...
 * arrays.
 */
int bmpReader::readMultipleImage(FILE *fp, RGB ***argbs, char ***xorMasks,
		      ...
		      int *imageCount)
{
    ...
    *imageCount = count;
	 ...
	 */
	filePos = ftell(fp);
	rc = endianConverter.readUINT16little(fp, &imageType);
	if (rc != 0)
	    return rc;
